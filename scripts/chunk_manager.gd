...
## Process the chunk generation queue (limited chunks per frame)
func _process_chunk_queue() -> void:
	...
	# Generate up to MAX_CHUNKS_PER_FRAME chunks
	var chunks_generated: int = 0
	while chunks_generated < MAX_CHUNKS_PER_FRAME and not chunk_generation_queue.is_empty():
		var chunk_pos: Vector3i = chunk_generation_queue.pop_front()

		# Skip if chunk already exists (might have been generated by initial load)
